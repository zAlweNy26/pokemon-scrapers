...
async function fetchPokemons(region: number, selector = 'table') {
    const url = `${homeUrl}/${regions[region].toLowerCase()}pokemon.shtml`
    ...
    rows.each((_i, row) => {
        ...
        const item: Pokemon = { index, subIndex, region, depositable, ndex, name, form, types }
        values.push(item)
...
for (const region of regions) {
    ...
    await fetchPokemons(index)
    console.log(`${region} :`, values.filter(p => p.region === index).length)
}

// Sort by index and then by subIndex
values.sort((a, b) => a.index === b.index ? a.subIndex - b.subIndex : a.index - b.index)

console.log('Total Pokémons :', values.length)
